...
- **🤖 AI 臉部辨識擴充 (`ai`, `ai/custom`)**：
  - **核心辨識**: 正式站 (`main`) 結合 FER 架構及**MediaPipe** (高效人臉偵測)，針對 6 大核心情緒（生氣、厭惡、害怕、開心、難過、驚訝）提供分析。本專案 (`dev`) 作為**測試站**，導入 **TensorFlow Lite (TFLite)** 進行模型量化與推論加速，取代原先龐大的 FER 模型。註：模型量化後準確率大幅下降，故正式站仍採取原有的 FER 架構
